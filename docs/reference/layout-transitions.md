...
Complete technical reference for layout transitions in Cascade Motion.

---

## Overview

Layout transitions automatically animate elements when their position or size changes. Cascade Motion uses the FLIP (First, Last, Invert, Play) technique to create smooth, performant layout animations.

---

## Hooks

### `useLayoutTransition(elementRef, config?): void`
...
**Returns:** Nothing. The hook applies animations directly to the element.

**Behavior:**

- Measures element bounds on each render using `useLayoutEffect`
- Compares current bounds with previous bounds
- Animates if change exceeds threshold (1px position, 1px size)
...
### `useSharedLayoutTransition(elementRef, config): void`
...
### `useBatchLayoutTransition(elementRefs, config?): void`
...
**Returns:** Nothing. The hook applies animations directly to the elements.
...
### `measureElement(element): BoundingBox`
...
**Returns:** A `BoundingBox` object with `x`, `y`, `width`, and `height` properties.

**Example:**
```typescript
import { measureElement } from '@cascade/motion-runtime';
...
### `measureElements(elements): BoundingBox[]`
...
**Returns:** Array of `BoundingBox` objects in the same order as input elements.

---

### `calculateTransformDelta(from, to): TransformDelta`
...
**Returns:** A `TransformDelta` object with `x`, `y`, `scaleX`, and `scaleY` properties.

**Example:**
```typescript
import { calculateTransformDelta } from '@cascade/motion-runtime';
...
### `detectLayoutChanges(elements, previousBounds): LayoutChange[]`
...
**Returns:** Array of `LayoutChange` objects for elements that changed significantly.
...
### `generateFLIPKeyframes(name, config): { css: string; className: string }`
...
**Returns:** Object with `css` (CSS string) and `className` (CSS class name).

**Example:**
```typescript
import { generateFLIPKeyframes } from '@cascade/motion-runtime';
...
1. **Batch Operations**: Use `useBatchLayoutTransition` for multiple elements instead of multiple `useLayoutTransition` hooks.

2. **Transform Origin**: Choose the transform origin that matches your use case. `'center'` is usually best.

3. **Disable When Not Needed**: Use the `enabled` flag to disable transitions when they're not needed.

4. **Avoid Rapid Changes**: Layout transitions work best when changes are intentional and spaced out.
...
1. **Check threshold**: Changes must exceed 1px position or 1px size
...
- [Layout Transitions Tutorial](../tutorials/layout-transitions.md) - Learn how to use layout transitions
- [How to Animate List Reorder](../how-to/animate-list-reorder.md) - Practical examples
- [How to Create Shared Element Transition](../how-to/create-shared-element-transition.md) - Shared element guide
